...
public class Payment {
    ...
    public static Payment authorize(String merchantId, String orderId, String customerId, Money money) {
        ...
        return new Payment(UUID.randomUUID(), merchantId, orderId, customerId, money,
                PaymentStatus.AUTHORIZED, null, now, now);
    }
    ...
    public void reject(String reason) {
        if (status == PaymentStatus.CAPTURED) {
            throw new IllegalStateException("Captured payments cannot be rejected");
        }
        requireText(reason, "reason");
        if (reason.length() > 250) throw new IllegalArgumentException("reason must have at most 250 characters");
